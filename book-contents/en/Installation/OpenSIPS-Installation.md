...
### Step 9: Setup rtpproxy

We need rtpproxy when we have User Agents (UA) under NAT routers.
...
Edit rtpproxy config file `/etc/default/rtpproxy` and make these changes:

- Uncomment `CONTROL_SOCK` option
- Edit `EXTRA_OPTS` option from `EXTRA_OPTS=""` to `EXTRA_OPTS="-l YOUR_SERVER_IP_ADDRESS_HERE"`. For example if the server IP address is `192.168.1.150` then it would become `EXTRA_OPTS="-l 192.168.1.150"`.
